...
class GlobalBarWidget(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self._last_import_dir = self.settings.value(
        ...
        self.menu_button.setIcon(load_icon("vertical_dots.svg"))
...
"""
    ...
    def _open_single_dialog(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(
            ...
            "All Video Files (*.mp4 *.mkv *.avi *.mov *.webm *.flv *.wmv *.m4v *.ts *.mts *.m2ts *.3gp *.mpg *.mpeg *.vob);;All Audio Files (*.mp3 *.aac *.wav *.flac *.ogg *.m4a *.wma *.opus *.alac);;All Files (*.*)",
        )
