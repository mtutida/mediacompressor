...
class AssistedImportDialog(QDialog):
    ...
    def __init__(self, video_count, audio_count, parent=None, files=None):
        ...
        if video_count:
            ...
            self.v_mode_video.currentIndexChanged.connect(lambda i: [self.v_compress.setChecked(i==0), self.v_convert.setChecked(i==1), self.v_extract.setChecked(i==2)])
            ...
            self._video_formats = ["MP4","MKV","WEBM","MOV","AVI","FLV","TS","M4V"]
            ...
            self.v_profile.addItems(["Economia Máxima","Equilíbrio","Qualidade Prioritária"])
        ...
        if True:
            ...
            self.a_bitrate.addItems(["96 kbps","128 kbps","160 kbps","192 kbps","256 kbps","320 kbps"])
    ...
    def update_header(self):
        ...
        for f in self.files:
            ...
            if ext in (".mp3",".aac",".wav",".flac",".ogg",".m4a",".wma",".opus",".alac"):
                audio += 1
            else:
                video += 1
        ...
        self.header.setText(f"{total} {file_label}: {video} {video_label} + {audio} {audio_label}")
    ...
    def _enqueue(self):
        ...
        payload = {
            ...
            "audio": {
                "mode": "convert" if getattr(self, "a_convert", None) and self.a_convert.isChecked() else "keep" if getattr(self, "a_keep", None) and self.a_keep.isChecked() else "remove" if getattr(self, "a_remove", None) and self.a_remove.isChecked() else None,
                "codec": getattr(self, "a_format", None).currentText() if getattr(self, "a_format", None) else None,
                "bitrate": getattr(self, "a_bitrate", None).currentText() if getattr(self, "a_bitrate", None) else None
            }
        }
